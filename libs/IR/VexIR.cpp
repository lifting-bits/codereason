            ...
            case OFFB_ACFLAG:
              ...
              reg.regclass = ProgramCounter;
						...
						case OFFB_GDT:
                            reg.Reg32 = GDT32;
                            break;
						case offsetof(VexGuestX86State, guest_EMWARN):
							reg.Reg32 = EMWARN32;
							break;
              ...
              break;
				...
				case 64:
					switch(guestOffset) {
                        ...
                        case offsetof(VexGuestX86State, guest_TISTART):
                            reg.Reg64 = TI;
                            break;
                        ...
                        case offsetof(VexGuestX86State, guest_TILEN):
                            break;
					}
					break;
			}
...
ExpressionPtr expressionBuilder(IRExpr *exp, TargetArch arch, BlockPtr b) {
	...
	switch(exp->tag) {
		...
		case Iex_Qop:
			e = new VExOp(b, VOpPtr(new VOp(exp->Iex.Qop.op)), expressionBuilder(exp->Iex.Qop.arg1, arch, b), expressionBuilder(exp->Iex.Qop.arg1, arch, b), expressionBuilder(exp->Iex.Qop.arg1, arch, b), expressionBuilder(exp->Iex.Qop.arg1, arch, b));
			break;

		case Iex_Triop:
			e = new VExOp(b, VOpPtr(new VOp(exp->Iex.Qop.op)), expressionBuilder(exp->Iex.Triop.arg1, arch, b), expressionBuilder(exp->Iex.Triop.arg2, arch, b), expressionBuilder(exp->Iex.Triop.arg3, arch, b));
			break;

		case Iex_Binop:
			e = new VExOp(b, VOpPtr(new VOp(exp->Iex.Qop.op)), expressionBuilder(exp->Iex.Binop.arg1, arch, b), expressionBuilder(exp->Iex.Binop.arg2, arch, b));
			break;

		case Iex_Unop:
			e = new VExOp(b, VOpPtr(new VOp(exp->Iex.Qop.op)), expressionBuilder(exp->Iex.Unop.arg, arch,b ));
			break;
		...
		case Iex_Mux0X:
			e = new VExMux0X(b, expressionBuilder(exp->Iex.Mux0X.cond, arch, b), expressionBuilder(exp->Iex.Mux0X.expr0, arch, b), expressionBuilder(exp->Iex.Mux0X.exprX, arch, b));
			break;
...
StatementPtr VBlock::statementBuilder(IRStmt *stmt, TargetArch arch) {
	...
	switch(stmt->tag) {
	   ...
	   case Ist_PutI:
		   st = new VStPut(shared_from_this(), stmt->Ist.PutI.descr, expressionBuilder(stmt->Ist.PutI.data, arch, shared_from_this()), arch, expressionBuilder(stmt->Ist.PutI.ix, arch, shared_from_this()), stmt->Ist.PutI.bias);
		   break;
