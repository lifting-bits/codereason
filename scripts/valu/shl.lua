...
function onPost(v)
    ...
    if vee.getexit(v) == Return then
        ...
        if  checkReg(v, EAX, 32, 42) or
            checkReg(v, EBX, 32, 42) or
            checkReg(v, ECX, 32, 42) or
            checkReg(v, EDX, 32, 42) or
            checkReg(v, ESI, 32, 42) or
            checkReg(v, EDI, 32, 42) or
            checkReg(v, EBP, 32, 42) then
            return true
        end
    end
